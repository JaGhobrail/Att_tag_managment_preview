...
export default function UrlDialog(props) {
    ...
    const [notes, setNotes] = useState(item.page_url)
    ...
    return (
        <>
            <Button onClick={handleOpenDialog} variant='text' size='small' color='primary'>{item.page_url}</Button>
            <Dialog
                fullWidth={true}
                maxWidth={'sm'}
                open={openDialog}
                    ...
                    <Toolbar className="flex w-full">
                        <div className='flex flex-col'>
                            <Typography variant="subtitle1" color="inherit">
                                LAUNCH PAGE URL TO INVESTIGATE
                            </Typography>
                        </div>
                    </Toolbar>
                </AppBar>
